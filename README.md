        ...
        public JsonResult SaveBFProdData(List<BF_Production> modelList)
        {
            try
            {
                ...
                WHEN MATCHED THEN
                    UPDATE SET 
                        ACTUAL = :ACT_ONDT,
                        REPORTED = :REPORT_ONDT,
                        BALANCE = :BALANCE
                WHEN NOT MATCHED THEN
                    INSERT (FURNACE, ACTUAL, REPORTED, BALANCE)
                    VALUES (:FURNACE, :ACT_ONDT, :REPORT_ONDT, :BALANCE)";

                        using (OracleCommand cmd = new OracleCommand(query, con))
                        {
                            cmd.Parameters.Add("FURNACE", model.FURNACE);
                            cmd.Parameters.Add("ACT_ONDT", model.ACT_ONDT);
                            cmd.Parameters.Add("REPORT_ONDT", model.REPORT_ONDT);
                            cmd.Parameters.Add("BALANCE", model.BALANCE);
