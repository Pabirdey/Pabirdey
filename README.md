    document.addEventListener("click",function(e){
            if (e.target.classList.contains("btngetLot")) {
                debugger;
                var row=e.target.closest("tr");
                var supplier = row.querySelector(".mgClayInput").innerText.trim();
                var selectedFurnace=document.getElementById("lstFur").value;
                var selectedDate = document.getElementById("hiddenDate").value;
                alert(selectedFurnace);
                alert(supplier);
                alert(selectedDate);               
            }
        });
         ...
         function Display_Mudgun_Details(date, furnace) {
               $.ajax({
                   url: '@Url.Action("Get_Display_Mudgun_Details", "HML")',
                   type: 'GET',
                       ...
                       for (let i = 0; i < parsedData.length; i++) {
                           ...
                           tableBody += `<td>
                                            <div class="mgClayWrapper">
                                                <input name="MG_CLAY_USED" type="text" class ="form-control form-control-sm mgClayInput" id="MG_CLAY_USED" value="${parsedData[i].MG_CLAY_USED}">
                           ...
                           tableBody += `<td class="d-flex gap-0"><input name="LOT_NO" class="LOT_NO" style="width:120px;"  value='${parsedData[i].LOT_NO}' />
                                        <button type="button" class ="btn btn-primary btn-sm btngetLot" data-castno="${parsedData[i].LOT_NO}">::</button></td>`;
