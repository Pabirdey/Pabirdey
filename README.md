...
BEGIN
  ...
  --      vMAXDATE := TO_DATE('01-JAN-2026 06:00:00','DD-MON-YYYY');
  --  END IF;
    ...
    FOR k IN (SELECT SL_NO, PLANT, COLUMN_NAME, SOURCE_TABLE,DESTINATION_TABLE, LCL, UCL FROM TSBSL.T_TSM_DRI_ALERT_MASTER ORDER BY SL_NO)
        LOOP
            BEGIN       
                ...
                END IF;        
        vSql :='SELECT COUNT(*) FROM ' || k.SOURCE_TABLE ||' WHERE TRUNC(TIMESTAMP)= TO_DATE(''' ||TO_CHAR(vSTDATE,'DD-MON-YYYY HH24:MI:SS') ||''',''DD-MON-YYYY HH24:MI:SS'')' ||' AND ' || k.COLUMN_NAME || ' < ' || k.LCL;     
        ...
        vSql :='SELECT COUNT(*) FROM ' || k.SOURCE_TABLE ||' WHERE TRUNC(TIMESTAMP)= TO_DATE(''' ||TO_CHAR(vSTDATE,'DD-MON-YYYY HH24:MI:SS') ||''',''DD-MON-YYYY HH24:MI:SS'')' ||' AND ' || k.COLUMN_NAME || ' > ' || k.UCL;
