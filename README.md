...
Begin  
    ...
    Begin  
        IF v_count > 0 THEN
            ...
            ORDER BY WEB_SL_NO;    
            */
            SELECT b.TAG_ID,b.WEB_COLUMN MATERIAL_NAME,CASE WHEN b.FUR_NAME = 'C' Then nvl(VALUE,0) AS VALUE_TON ,nvl(VALUE,0)*1000 AS VALUE_KG  
            Else nvl(VALUE,0)/1000 AS VALUE_TON ,nvl(VALUE,0) AS VALUE_KG  END
            FROM imtg.T_ATOF_BIN_DETAILS b ,TEST.T_FURNACES_PROC_HM_PROD a Where b.TAG_ID = a.TAG_ID(+)
            AND b.REQUIRED = 'Y' AND b.WEB_COLUMN IS NOT NULL AND a.TIMESTAMP(+) =p_date AND b.FUR_NAME =p_furnace
            ORDER BY WEB_SL_NO; 
