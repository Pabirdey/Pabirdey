        ...
        public ActionResult RmbbPile(RmbbPile input, string SaveRawMaterial)
        {
            ...
            if (!string.IsNullOrEmpty(SaveRawMaterial))
            {
                ...
                using (OracleConnection conn = new OracleConnection(iMonitorWebUtils.msConRWString))
                {
                    ...
                    decimal? LD_SLUDGE = null, MILL_SCALE = null, FLUE_DUST = null, ESP_DUST = null,
                     KILN_DUST = null, GCP_SLUDGE = null, LIME_FINES = null, WRP = null,
                     LD_SLAG_FINES = null, MILL_SLUDGE = null;
    ...
    WHERE TRIM(SOURCE) = 'RMBB_KNR' AND TIMESTAMP >= TRUNC(:prepEndDate) - 200 AND BLEND_NO = :BL1                   
                    ...
                    using (OracleCommand blendCmd = new OracleCommand(blendQuery, conn))
                    {
                        ...
                        using (OracleDataReader reader = blendCmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                model.LD_SLUDGE = reader.IsDBNull(0) ? (decimal?)null : reader.GetDecimal(0);
                                model.MILL_SCALE = reader.IsDBNull(1) ? (decimal?)null : reader.GetDecimal(1);
                                model.FLUE_DUST = reader.IsDBNull(2) ? (decimal?)null : reader.GetDecimal(2);
                                model.ESP_DUST = reader.IsDBNull(3) ? (decimal?)null : reader.GetDecimal(3);
                                model.KILN_DUST = reader.IsDBNull(4) ? (decimal?)null : reader.GetDecimal(4);
                                model.GCP_SLUDGE = reader.IsDBNull(5) ? (decimal?)null : reader.GetDecimal(5);
                                model.LIME_FINES = reader.IsDBNull(6) ? (decimal?)null : reader.GetDecimal(6);
                                model.WRP = reader.IsDBNull(7) ? (decimal?)null : reader.GetDecimal(7);
                                model.LD_SLAG_FINES = reader.IsDBNull(8) ? (decimal?)null : reader.GetDecimal(8);
                                model.MILL_SLUDGE = reader.IsDBNull(9) ? (decimal?)null : reader.GetDecimal(9);
                            }
                        }
                    }
                    ...
                    using (OracleCommand checkCmd = new OracleCommand(checkSql, conn))
                    {
                        ...
                        if (count > 0)
                        {
                            ...
                            string updateSql = @"UPDATE Test.T_PILE_RAWMAT_QUANTITY_NEW SET                                         
                                        ...
                                        LD_SLUDGE_FRESH=:LD_SLUDGE_FRESH,
                            ...
                            using (OracleCommand updateCmd = new OracleCommand(updateSql, conn))
                            {
                                ...
                                updateCmd.Parameters.Add(":LD_SLUDGE_FRESH", OracleDbType.Decimal).Value = input.LD_SLUDGE_FRESH ?? (object)DBNull.Value;
                    ...
                    using (OracleCommand cmd = new OracleCommand(query, conn))
                    {
                        ...
                        using (OracleDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                model.LD_SLUDGE_FRESH = reader["LD_SLUDGE_PRM"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["LD_SLUDGE_PRM"]);
