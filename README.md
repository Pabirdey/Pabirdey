                ...
                try
                {
                    ...
                    for (int i = 0; i < modelList.Count; i++)
                    {
                        ...
                        using (OracleCommand cmd = new OracleCommand(checkQuery, con))
                        {
                            ...
                            cmd.BindByName = true;
                            cmd.Parameters.Add("FURNACE", OracleDbType.Varchar2).Value = bfModel.FURNACE;
                            cmd.Parameters.Add("TIMESTAMP", OracleDbType.Date).Value = Convert.ToDateTime(bfModel.PROD_DATE);
                        ...
                        if (count > 0)
                        {
                            ...
                            using (OracleCommand cmd = new OracleCommand(updateQuery, con))
                            {
                                cmd.Transaction = trans;
                                cmd.BindByName = true;
                                cmd.Parameters.Add("ACTUAL", OracleDbType.Decimal).Value = bfModel.ACTUAL;
                                cmd.Parameters.Add("REPORTED", OracleDbType.Decimal).Value = bfModel.REPORTED;
                                cmd.Parameters.Add("BALANCE", OracleDbType.Decimal).Value = bfModel.BALANCE;
                                cmd.Parameters.Add("FURNACE", OracleDbType.Varchar2).Value = bfModel.FURNACE;
                                cmd.Parameters.Add("TIMESTAMP", OracleDbType.Date).Value = Convert.ToDateTime(bfModel.PROD_DATE);
                                cmd.ExecuteNonQuery();
                            }
                        ...
                        int VCount = 0;
                        string countQry = @"SELECT COUNT(*) FROM DEMO.T_LADLE WHERE DATE_TIME =:TIMESTAMP AND PLANT='A-F'";
                        using (OracleCommand cmd = new OracleCommand(countQry, con))
                        {
                            cmd.Transaction = trans;
                            cmd.Parameters.Add("TIMESTAMP", OracleDbType.Varchar2).Value = Convert.ToDateTime(bfModel.PROD_DATE);
                            VCount = Convert.ToInt32(cmd.ExecuteScalar());
                        }

                        if (VCount > 0)
                        {
                            string updateQry = @"UPDATE DEMO.T_LADLE
                                            SET                                                
                                                LD1_TONS = :LD1_TONS,
                                                LD2_TONS = :LD2_TONS,
                                                LD3_TONS = :LD3_TONS,
                                                ...
                                                AND PLANT='A-F'";
                            using (OracleCommand cmd = new OracleCommand(updateQry, con))
                            {
                                cmd.Transaction = trans;
                                cmd.Parameters.Add("DATE_TIME", OracleDbType.Date).Value = Convert.ToDateTime(bfModel.PROD_DATE);
                                cmd.Parameters.Add("LD1_TONS", OracleDbType.Decimal).Value = bfModel.LD1_TONS;
                                cmd.Parameters.Add("LD2_TONS", OracleDbType.Decimal).Value = bfModel.LD2_TONS;
                                cmd.Parameters.Add("LD3_TONS", OracleDbType.Decimal).Value = bfModel.LD3_TONS;
                                cmd.Parameters.Add("MRDTP_TONS", OracleDbType.Decimal).Value = bfModel.MRDTP_TONS;
                                cmd.Parameters.Add("NOOFTP", OracleDbType.Decimal).Value = bfModel.NOOFTP;
                                cmd.ExecuteNonQuery();
                            }
                        }
                        else
