    ...
    $.ajax({
        ...
        success: function (result_Mudgun_Details) {
            ...
            for (var i = 0; i < parsedData.length; i++) {
                ...
                // ✅ MG Clay Type with onchange to open modal
                tableBody += `<td>
                ...
                tableBody += `<td class="d-flex gap-0">
                    ...
                    <button type="button" class="btn btn-primary btn-sm getLot" data-castno="${parsedData[i].LOT_NO}">::</button>
                </td>`;
                ...
                tableBody += `<td>
                    <select name="MUDGUN_NOZZLE" class="form-select form-select-lg">
                        ...
                        <option value="REPLACEMENT" ${parsedData[i].MUDGUN_NOZZLE === 'REPLACEMENT' ? 'selected' : ''}>REPLACEMENT</option>
                        <option value="WEILD" ${parsedData[i].MUDGUN_NOZZLE === 'WEILD' ? 'selected' : ''}>WEILD</option>
                    </select>
                </td>`;
...
function selectItem(el, rowIndex) {
    ...
    if (value === "OTHERS") {
        ...
        $("#txtOtherMGClay").val("");
        let modal = new bootstrap.Modal(document.getElementById("mgClayOtherModal"));
        modal.show();
        return;
    }
...
function saveOtherMGClay() {
    ...
    var otherValue = $("#txtOtherMGClay").val().trim();
    if (otherValue === "") {
        alert("Please enter MG Clay Type");
        return;
    }
    $("#clayInput_" + rowIndex).val(otherValue);
    $("#mgClayOtherModal").modal("hide");

    // OPTIONAL: send to controller
    /*
    $.ajax({
        url: '@Url.Action("SaveOtherMGClay", "CastHouse")',
        type: 'POST',
        data: {
            CastNo: $("#Mudgun_Details tbody tr").eq(rowIndex).data("castno"),
            MG_CLAY_USED: otherValue
        }
    });
    */
}
